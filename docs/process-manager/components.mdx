...
| response `*` | object | undefined | ❌ | Path to final result. This will be the output. |
| info\[1-3\] `**` | string | undefined | ❌ | Extra information to analyze in process mining. |
...
> `*` *There are a few ways to return data back to the client that starts the process such as inputPath, parameters and response.*

> `**` *Process mining covers pre-defined attributes such as processId, executionId, stepId, etc. With these info parameters, you can pass 3 more data to process mining data pool: info1, info2 and info3.*
...
RBS has various microservices which are available through middleware (RBS Core).
...
> You should provide one of these attributes `inputPath`, `parameters` or `each` with `action` attribute to send a payload in your request.

> If the service return an error when `retry` is enabled, Process Manager retries the request with an exponential backoff according to your configuration.
Do not forget that timeout and retry might end up a race condition.

> If step fails when `rollbackOnFailure` is enabled, Process Manager calls every `actionRollback` executed before failed step.

> If you want to call another process or you don't want to send metadata of current process in a service call, you should set `plain` as true. Default value for *plain* is false.

> If you want to send metadata of a process in a service call, you should set processId and executionId in `headers` attribute.
...
On the other hand, you can make a service call on user's behalf by using `auth` attribute.
This feature has two ways of usages. In one of them, you should generate a valid custom token for a specific user and use it in *auth* attribute with `token` parameter. Otherwise you should provide `userId`, `roles` and `claims` to generate a token before making the request.

```typescript
const SynchronousServiceCallExample = {
    id: 'GET_ORDER',
...
> If you need an extra information in a step other than the first step, you can use `actionCallback` without providing an action. In that case, instead of making a service call, Process Manager awaits for an asynchronous response from a client instance.

```typescript
const AsynchronousServiceCallExample = {
    id: 'PAYMENT',
...
```typescript
const StartAnotherProcessExample = {
...
> You should provide one of these attributes `inputPath` or `parameters` to send an initial payload for the process.
...
```typescript
const JS_WithCustomFunctionExample = {
    id: 'JAVASCRIPT',
    js: '_.fnTest($.items)',
    inputPath: '$.payload',
    resultPath: 'groupResult',
...
```typescript
const MultiFactorAuthenticationSecret =  {
    id: 'VERIFY',
...
```typescript
const MultiFactorAuthenticationVerify = {
    id: 'VERIFY',
