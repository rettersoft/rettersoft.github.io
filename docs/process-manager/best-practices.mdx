...
```typescript
const GetOrderNotRecommendedWay  = {
    id: 'GET_ORDER',
...
> If `GetOrder` fails, process should continue with `Loop` step until the current value of `retry` is greater than *3*. In initial state or one of the earlier steps of process, we should set `retry` as *0*.
