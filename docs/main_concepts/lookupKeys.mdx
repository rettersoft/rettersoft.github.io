...
export async function updateEmail(data: Data): Promise<StepResponse> {
    ...
    if(data.method.request.body.code !== data.state.private.emailValidationCode) {
        data.method.response = {statusCode: 403}
        return data
    }
...
export async function findUserByEmail(data: Data): Promise<StepResponse> {
    ...
    const lookupResult = await sdk.getLookUpKey({
        key: {
